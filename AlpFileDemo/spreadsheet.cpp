...
//    qDebug()<<alp->writeChannelData(entry3,channel3,data3);
  ...
  /***********************************写入流信息*****************************************/
    ALP_OBJECT_ENTRY *entryStream = AlpData::entry("image", AlpData::normal,AlpData::StreamAttribure,
                                              AlpData::image);

    QString fileNameImage = QFileDialog::getOpenFileName(this, tr("Open File"),
                                                    QDir::homePath(),
                                                    tr("Images (*.png *.xpm *.jpg)"));
//    QImage *imageStream = new QImage(fileNameImage);
    QFile file(fileNameImage);
    ...
    qDebug()<<write->writeStreamData(entryStream, tempData, filesize);
}
...
void Sheet::doubleClicked(QListWidgetItem *itemClick)
{
    ...
    if (itemClick->data(Qt::ToolTipRole).toInt() == AlpData::CurveObject) {
        ...
        setHorizontalHeaderLabels(horizontalHeaderLabel);
        setVerticalHeaderLabels(verticalheaderLabel);
    } else if (itemClick->data(Qt::ToolTipRole).toInt() == AlpData::StreamObject) {
        MetaData *meta = reader.meta(itemClick->text());
        char * streamData = static_cast<char *>(reader.data(itemClick->text()));
        int len =  meta->property("objectLength").toInt();
        QImage image = QImage::fromData((uchar *)streamData,len);
